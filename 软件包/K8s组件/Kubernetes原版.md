...
```bash
vi /etc/hosts
172.28.142.164 test-nec-k8s-m1
172.28.142.192 test-nec-k8s-m2
172.28.142.196 test-nec-k8s-m3
172.28.142.202 test-nec-k8s-n1
172.28.142.209 test-nec-k8s-n2
...
[root@test-nec-k8s-m1 bin]# for i in 164 192 196 202 209 219 ;do scp /opt/kube/ssl/ root@172.28.142.$i:/opt/kube/ssl/ ;done
...
crictl pull test-harbor.nec-k8s.com.cn/kube-system/pause-amd64:3.1
```
...
kubectl config set-cluster kubernetes \
 ...
 --embed-certs=true \
 --server=https://172.28.142.159:8443 \
...
listen-peer-urls: 'https://172.28.142.164:2380'
listen-client-urls: 'https://172.28.142.164:2379,https://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://172.28.142.164:2380'
advertise-client-urls: 'https://172.28.142.164:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
initial-cluster: 'test-nec-k8s-m1=https://172.28.142.164:2380,test-nec-k8s-m2=https://172.28.142.192:2380,test-nec-k8s-m3=https://172.28.142.196:2380'
...
##查看状态
etcdctl --endpoints=172.28.142.164:2379,172.28.142.192:2379,172.28.142.196:2379 \
...
listen kube-api
        ...
        option tcplog
        balance roundrobin
        server test-nec-k8s-m1 172.28.142.164:6443 check inter 2000 fall 2 rise 2 weight 1
...
global_defs {
    router_id lb-master-172.28.142.164
}
...
ExecStart=/opt/kube/bin/kube-apiserver \
      ...
      --secure-port=6443  \
      --insecure-port=0  \
      --advertise-address=172.28.142.164 \
      --service-cluster-ip-range=10.250.0.0/16  \
      --service-node-port-range=30000-40000  \
      --etcd-servers=https://172.28.142.164:2379,https://172.28.142.192:2379,https://172.28.142.196:2379 \
...
Environment="KUBELET_CONFIG_ARGS=--config=/etc/kubernetes/kubelet-conf.yml --pod-infra-container-image=test-harbor.nec-k8s.com.cn/kube-system/pause-amd64:3.1 "
...
cp /opt/software/release-v3.22.2/bin/calicoctl /usr/bin/
...
NAME              ASN       IPV4                IPV6
test-nec-k8s-m1   (64512)   172.28.142.164/25
test-nec-k8s-m2   (64512)   172.28.142.192/25
test-nec-k8s-m3   (64512)   172.28.142.196/25
test-nec-k8s-n1   (64512)   172.28.142.202/25
test-nec-k8s-n2   (64512)   172.28.142.209/25
...
listen ingress-http
        ...
        option tcplog
        balance roundrobin
        server test-nec-k8s-n1 172.28.142.202:80 check inter 2000 fall 2 rise 2 weight 1
        server test-nec-k8s-n1 172.28.142.209:80 check inter 2000 fall 2 rise 2 weight 1
...
listen ingress-https
        ...
        option tcplog
        balance roundrobin
        server tmci-k8s-node-01 172.28.142.202:443 check inter 2000 fall 2 rise 2 weight 1
        server tmci-k8s-node-01 172.28.142.209:443 check inter 2000 fall 2 rise 2 weight 1
